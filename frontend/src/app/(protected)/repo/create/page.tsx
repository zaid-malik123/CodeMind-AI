            ...
            {!isProcessing ? (
              ...
              >
                <div className="mb-8 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-0.5">Index your codebase contexts into CodeMind AI workspace.</p>
                  </div>
                </div>

                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="space-y-2">
                    <label className="text-xs font-semibold tracking-wider text-muted-foreground uppercase">Repository Name</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-semibold tracking-wider text-muted-foreground uppercase">Git Repository URL</label>
                    <input
                      type="url"
                      required
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-[10px] font-mono tracking-widest text-primary font-bold uppercase bg-primary/10 px-2.5 py-1 rounded-full">Pipeline Active</span>
                    <h3 className="text-lg font-bold mt-2.5 text-ellipsis overflow-hidden max-w-xs">{repoName}</h3>
                  </div>
                  <div className="text-right">
                    <span className="text-2xl font-black text-primary font-mono">{currentConfig.percent}%</span>
                  </div>
                </div>
                ...
                <div className="relative h-2 w-full rounded-full bg-muted overflow-hidden">
                  <motion.div
                    className={`h-full rounded-full ${
                      currentStatus === REPO_STATUS.REPO_FAILED ? "bg-destructive" : "bg-primary"
                    }`}
                    initial={{ width: "0%" }}
                ...
                <div className="space-y-1 pt-3 border-t border-border/60 text-sm">
                  <StatusItem label="Repository Fetch & Verification" isActive={currentStatus !== REPO_STATUS.REPO_PENDING} icon={<GitBranch size={16} />} status={currentStatus} />
                  <StatusItem label="AST Structure & Security Audit" isActive={currentConfig.percent > 25} icon={<Terminal size={16} />} status={currentStatus} />
                  <StatusItem label="Semantic Text Optimization (Chunking)" isActive={currentConfig.percent > 45} icon={<Cpu size={16} />} status={currentStatus} />
                  <StatusItem label="High-Dimensional Embeddings Synthesis" isActive={currentConfig.percent > 65} icon={<Database size={16} />} status={currentStatus} />
                </div>
                ...
                {(currentStatus === REPO_STATUS.REPO_READY || currentStatus === REPO_STATUS.REPO_FAILED) && (
                  <motion.button
                    ...
                    className={`w-full py-3.5 mt-2 rounded-xl font-semibold border text-center text-sm transition-all shadow-sm ${
                      currentStatus === REPO_STATUS.REPO_READY 
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-muted/30 border border-border/40 rounded-2xl p-6 space-y-5">
            <h3 className="text-xs font-bold tracking-wider text-muted-foreground uppercase flex items-center gap-1.5">
              <HelpCircle size={14} className="text-primary" /> How Context Processing Works
            </h3>
            ...
            <div className="space-y-4">
              <div className="flex gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold">1. Vector Embeddings Generation</h4>
                  <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">
                    Aapka poora code intelligent semantic chunks mein break hota hai taaki CodeMind AI aapke functions ka context samajh sake.
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold">2. Secure Localized Parsing</h4>
                  <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">
                    Repository vectors safe isolated vector engine storage space mein securely store kiye jaate hain.
                  </p>
                </div>
              </div>
            </div>
          ...
          <p className="text-[11px] text-muted-foreground/60 px-2 text-center lg:text-left">
            Need help configuring custom enterprise repositories? <span className="text-primary cursor-pointer hover:underline">Read our integration guidelines</span>.
          </p>
        </div>
...
const StatusItem = ({ label, isActive, icon, status }: { label: string; isActive: boolean; icon: React.ReactNode; status: RepoStatusType }) => {
  const isFailed = status === REPO_STATUS.REPO_FAILED;
  
  return (
    <div className={`flex items-center justify-between p-2.5 rounded-xl transition-all ${isActive ? "bg-muted/30 text-foreground" : "text-muted-foreground/50"}`}>
      <div className="flex items-center gap-3 min-w-0">
        <div className={`${isActive ? "text-primary" : "text-muted-foreground/30"}`}>{icon}</div>
        <span className={`text-xs truncate ${isActive ? "font-medium" : "font-normal"}`}>{label}</span>
      </div>
