...
class RepoController {
  ...
  deleteRepo = asyncHandler( async (req: AuthenticatedRequest, res: Response) => {
    const { repoId } = req.params as { repoId: string };
    const userId = req.user!._id.toString();

    await repoService.deleteRepo({ repoId, userId });

    res
      .status(HTTP_STATUS.OK)
      .json(new ApiResponse(HTTP_STATUS.OK, MESSAGES.REPO_DELETED));
